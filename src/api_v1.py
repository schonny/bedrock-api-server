...
@api.route('/get-online-server-version', methods=['GET', 'POST'])  # 1010
...
@api.route('/download-server', methods=['POST'])  # 1011
def download_server():
    try:
...
@api.route('/create-server', methods=['POST'])  # 1012
def create_server():
    try:
...
@api.route('/remove-server', methods=['POST'])  # 1013
def remove_server():
    try:
...
@api.route('/start-server', methods=['POST'])  # 1014
def start_server():
    try:
        properties = request.json
    except Exception as e:
...
@api.route('/stop-server', methods=['POST'])  # 1015
def stop_server():
    try:
        server_name = request.json.get('server-name')
    except Exception as e:
...
@api.route('/say-to-server', methods=['POST'])  # 1016
def say_to_server():
    try:
        server_name = request.json.get('server-name')
...
@api.route('/send-command', methods=['POST'])  # 1017
def send_command():
    try:
        server_name = request.json.get('server-name')
...
@api.route('/get-worlds', methods=['POST'])  # 1018
def get_worlds():
    try:
        server_name = request.json.get('server-name')
    except Exception as e:
...
@api.route('/create-backup', methods=['POST'])  # 1019
def create_backup():
    try:
        server_name = request.json.get('server-name')
...
@api.route('/restore-backup', methods=['POST'])  # 1020
def restore_backup():
    try:
        backup_name = request.json.get('backup-name')
        server_name = request.json.get('server-name')
...
@api.route('/remove-world', methods=['POST'])  # 1021
def remove_world():
    try:
        server_name = request.json.get('server-name')
        level_name = request.json.get('level-name')
...
@api.route('/remove-backup', methods=['POST'])  # 1022
def remove_backup():
    try:
        backup_name = request.json.get('backup-name')
    except Exception as e:
...
@api.route('/get-server-details', methods=['POST'])  # 1023
